...
def throw_cubes_game(chat_id, bet_amount):
    ...
    if current_balance is None or bet_amount > current_balance:
        ...
        bot.send_message(chat_id, "❌ You don't have enough 💰 for this bet!",reply_markup=markup)
    ...
    if player_roll > bot_roll:
        ...
        bot.send_message(
            chat_id,
            f"🎉 You *win* {winnings}💰! Your new balance: {new_balance} 💰",
            parse_mode="Markdown",
            reply_markup=markup
        )
        ...
        bot.send_message(
            chat_id,
            f"😢 The bot wins! You lose: {bet_amount}💰 🤧Your new balance: {new_balance} 💰",
            parse_mode="Markdown",
            reply_markup=markup
        )
def play_roulette(chat_id, bet_amount, category):
    ...
    if won:
        ...
        new_balance = current_balance + winnings
        update_balance(chat_id, new_balance)
        return f"You WON {winnings}💰! Result: {roulette_result} 🎉. Your new balance: {new_balance} 💰", new_balance
    else:
        new_balance = current_balance - bet_amount
        update_balance(chat_id, new_balance)
        return f"You LOST {bet_amount}. 😢 Result: {roulette_result} . Your new balance: {new_balance} 💰", new_balance
...
def start(message):
    ...
    text = f"Welcome, {first_name}! I'm MarqBot. Nice to meet you! 👋"
    ...
    if photos.total_count > 0:
        ...
        bot.send_photo(chat_id, avatar_file_id, caption=text)
    else:
...
def handle_callbacks(call):
    if call.data == 'casino':
        ...
        if bet_amount > current_balance:
            bot.send_message(call.message.chat.id,f"❌ You don't have enough funds to place this bet! Current balance: {current_balance} 💰. Please enter a lower amount.")
            bet_menu(call.message.chat.id, category)
            return
    ...
    try:
        ...
        for chat_id in chat_ids:
            try:
                bot.send_message(
                    chat_id[0],
                    "Yo, i'm going to sleep, see you later hommie ✋🏻",
                    parse_mode="Markdown"
                )
            except Exception as e:
